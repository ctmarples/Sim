...
from settings import (
    ...
    WILDLIFE_CONDITION_FED_GAIN,
    WILDLIFE_CONDITION_HUNGER_LOSS,
    WILDLIFE_CONDITION_BREED_MIN,
    WILDLIFE_IMMIGRATION_RABBIT,
    WILDLIFE_IMMIGRATION_WOLF,
    BIRD_MAX_POPULATION,
    BIRD_RECRUIT_CHANCE,
    TREE_LIFESPAN_YEARS,
...
BALANCE_CATEGORIES: tuple[BalanceCategory, ...] = (
    BalanceCategory(
        ...
        (
            ...
            BalanceParam(
                ...
                DAY_NIGHT_ENABLED, 0, 1, 1,
                "0 disables the lighting tint; 1 enables it in either calendar model.",
            ),
            BalanceParam(
                "FLEXIBLE_DAY_SECONDS_AT_X1", "Flexible day length at ×1", "float",
                FLEXIBLE_DAY_SECONDS_AT_X1, 5.0, 600.0, 5.0,
                "Used only by flexible calendars. Default is a five-minute day/night cycle.",
                "s",
            ),
            BalanceParam(
    ...
    BalanceCategory(
        ...
        (
            ...
            BalanceParam(
                "WILDLIFE_CONDITION_FED_GAIN",
                "Condition gain when fed",
                "float",
                WILDLIFE_CONDITION_FED_GAIN,
                0.0,
                1.0,
                0.05,
                "How fast colony/predator/bird condition rises toward 1 when food is available.",
            ),
            BalanceParam(
                "WILDLIFE_CONDITION_HUNGER_LOSS",
                "Condition loss when hungry",
                "float",
                WILDLIFE_CONDITION_HUNGER_LOSS,
                0.0,
                1.0,
                0.05,
                "How fast condition falls toward 0 without food.",
            ),
            BalanceParam(
                "WILDLIFE_CONDITION_BREED_MIN",
                "Min condition to breed",
                "float",
                WILDLIFE_CONDITION_BREED_MIN,
                0.0,
                1.0,
                0.05,
                "Colonies and predator packs need at least this condition to reproduce.",
            ),
            BalanceParam(
                "WILDLIFE_IMMIGRATION_RABBIT",
                "Rabbit immigration",
                ...
                1.0,
                0.05,
                "Spring chance an extinct rabbit colony recolonises suitable habitat.",
            ),
            BalanceParam(
                "WILDLIFE_IMMIGRATION_WOLF",
                "Wolf immigration",
                ...
                1.0,
                0.05,
                "Spring chance an extinct wolf population returns (slow).",
            ),
            BalanceParam(
                "BIRD_MAX_POPULATION",
                "Bird population cap / species",
                "int",
                float(BIRD_MAX_POPULATION),
                0,
                20,
                1,
                "Hard cap per hawk/owl species before prey scaling.",
            ),
            BalanceParam(
                "BIRD_RECRUIT_CHANCE",
                "Bird spring recruit chance",
                "float",
                BIRD_RECRUIT_CHANCE,
                0.0,
                1.0,
                0.05,
                "Spring chance to add a bird when prey and nest habitat are available.",
            ),
            BalanceParam(
                "WILDLIFE_BEE_FORAGE_PER_LEVEL",
            ...
            BalanceParam(
                ...
                1.0,
                0.05,
                "Chance a pack with both sexes adds a pup once each spring "
                "while fed, in condition, and under the population cap.",
            ),
            BalanceParam(
                "WOLF_HUNT_BOAR_MIN",
                "Wolves to hunt boar",
            ...
            BalanceParam(
                ...
                1.0,
                0.05,
                "Chance a fox pack with both sexes adds a pup once each spring "
                "while fed, in condition, and under the population cap.",
            ),
        ),
    ),
