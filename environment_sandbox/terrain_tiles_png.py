"""PNG terrain fills + soft neighbour joins (archived / optional).

Not the active default. Switch with ``settings.TERRAIN_FILL_MODE = "png"``.
Active path is ``terrain_tiles_procedural`` via ``terrain_tiles``.
...
_STEM: dict[TerrainType, str] = {
    ...
    TerrainType.FOREST_FLOOR: "soil",  # reuse soil PNGs if present; colour via fill
