        ...
        *,
        ...
        if not buildings and not construction_sites:
            ...
            for b in buildings.values():
                ...
                if selected and b.is_field_plot and b.plans:
                    for plan in b.plans:
...
def draw_feature(
    ...
    if feature == FeatureType.TREE:
        ...
        if feature == FeatureType.BERRY_BUSH:
            from berry_bushes import berry_bush_colour, berry_empty_fruit_colour, berry_fruit_colour

            immature = int(tree_age_years) < 1
            bush_rgb = adjust_colour(berry_bush_colour(crop_kind), vibrancy)
            if immature:
                blit_icon(
                    surface,
                    ICON_BERRY_BUSH_SAPLING,
                    ...
                    size,
                    variant=v,
                    recolour={"bush": bush_rgb},
                )
            else:
                berry_colour = (
                    adjust_colour(berry_fruit_colour(crop_kind), vibrancy)
                    if deposit > 0
                    else adjust_colour(berry_empty_fruit_colour(crop_kind), vibrancy)
                )
                blit_icon(
                    surface,
                    ICON_BERRY_BUSH,
                    ...
                    size,
                    variant=v,
                    recolour={"bush": bush_rgb, "berry": berry_colour},
                )
        else:
        ...
        from berry_bushes import (
            BERRY_BUSH_KEYS,
            berry_bush_colour,
            berry_empty_fruit_colour,
            berry_fruit_colour,
        )

        # Orchard / permanent bushes planted as farm crops use bush art.
        if feature == FeatureType.CROP_HERB and (crop_kind or "") in BERRY_BUSH_KEYS:
            immature = growth_ticks > 0
            bush_rgb = adjust_colour(berry_bush_colour(crop_kind), vibrancy)
            if immature:
                blit_icon(
                    surface,
                ...
                berry_colour = (
                    adjust_colour(berry_empty_fruit_colour(crop_kind), vibrancy)
                    if deposit < 0
                    else adjust_colour(berry_fruit_colour(crop_kind), vibrancy)
                )
                blit_icon(
                    surface,
                    ICON_BERRY_BUSH,
