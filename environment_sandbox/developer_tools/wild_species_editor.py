...
def _base_registry():
    ...
    if _BASE is None:
        ...
        for item in wild_species.WILD_SPECIES:
            ...
            normalized.append(replace(item,icon_base=icon,terrains=(item.terrains,) if isinstance(item.terrains,str) else item.terrains,edge_terrains=(item.edge_terrains,) if isinstance(item.edge_terrains,str) else item.edge_terrains))
...
def reload_wild_species(path=PATH) -> ReloadResult:
    ...
    _apply_registry(items);REVISION+=1
...
class WildSpeciesEditorService:
    def __init__(self,root=None):
        self.root=Path(root or get_content_root());self.path=self.root/"objects_data"/"wild_species_overrides.json";self.candidate=None;self.original=None;self.candidate_slots=[0,1,3,4];self.original_slots=[0,1,3,4];self.overrides={};self.report=ValidationReport();self.load()
    @property
    def records(self):
        import wild_species
        return list(wild_species.WILD_SPECIES)
    @property
    def dirty(self):return self.candidate is not None and (self.candidate!=self.original or self.candidate_slots!=self.original_slots)
    def load(self):
    ...
    def select(self,item):
        self.original=item;self.candidate=item;slots=self.overrides.get(item.key,{}).get("_footprint_slots",[0,1,3,4]);self.original_slots=list(slots);self.candidate_slots=list(slots);self.report=validate_species(item)
    def update(self,**values):
        self.candidate=replace(self.candidate,**values);self.report=validate_species(self.candidate);return self.report
    def cancel(self):self.candidate=self.original;self.candidate_slots=list(self.original_slots);self.report=validate_species(self.candidate) if self.candidate else ValidationReport()
    def save(self):
        global REVISION
        ...
        self.overrides=proposed;_apply_registry(items)
        self.original=next(x for x in items if x.key==self.candidate.key);self.candidate=self.original;self.original_slots=list(self.candidate_slots);self.report=whole_report
        REVISION+=1
